...
## Example 10.1 Spatial patterns in lead concentrations {-}
...
## Example 10.2: Examining the log concentrations of lead {-}
...
## EXTRA: Example 10.1: Spatial patterns of benzene concentrations in Montreal, QC, Canada {-}

Map the locations of the monitoring stations in Montreal.

This example uses the [montreal_benzene_apr.csv](https://github.com/spacetime-environ/stepi2/blob/main/data/montreal_benzene_apr.csv)
...
## EXTRA: Example 10.2: Examining the log concentrations of benzene in Montreal {-}
...
This example uses the coordinates in the [NY_metadata.txt](https://github.com/spacetime-environ/stepi2/blob/main/data/NY_metadata.txt) file.
...
no2 <- read.xls("data/NO2Europe.xlsx", perl = perl)
...
no2_utm <-
  ...
  select(utm_x, utm_y, Altitude, Country, AirPollutionLevel)  |> 
 mutate( ID = c(1:896))
...
mcmc.out <- readRDS("data/mcmc_out.rds")
```
...
grid <- sf::st_read("data/grid/10km_grid.shp")
covs_grid <- read.csv("../data/grid/centroids_altitude.csv")
...
covs_grid_utm <-
  ...
  mutate(utm_x = get_utm(x, y, zone2, loc = "x"),
         utm_y = get_utm(x, y, zone2, loc = "y"))|> as.data.frame() |> 
  select(id, utm_x, utm_y, starts_with("eu_dem_v11")) |> 
...
obsMu <- cbind(1, easting_scaled, northing_scaled, altitude_transf_scaled) %*% t(cbind(post_beta0, post_beta1, post_beta2, post_beta3))
...
Rcpp::sourceCpp("prediction_marginal_gp12.cpp")
...
ggplot() + geom_sf(data = cut_grid, colour = NA, aes(fill = pred)) + 
  ditch_the_axes + scale_fill_viridis_c(direction = -1, name = "log NO2 concentration") 
``` 
...
For this example we are using the same $\texttt{gambia}$ data set from the $\texttt{geoR}$ package but an $\texttt{id_area}$ column was added using QGIS to differentiate the different areas as in the original paper. Therefore, we need to load the [gambia_area.csv](https://github.com/spacetime-environ/stepi2/blob/main/data/gambia_area.csv) file.
